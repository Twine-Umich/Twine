...
abstract class Arg(in_uniqueId: Long = -1) {
  def fullName(ctx: Component): String = name
  def name: String
  var uniqueId: Long = in_uniqueId
}

case class Node(id: HasId, in_uniqueId: Long = -1) extends Arg(in_uniqueId) {
...
case class Ref(name: String, in_uniqueId: Long = -1) extends Arg(in_uniqueId)
case class ModuleIO(mod: BaseModule, name: String, in_uniqueId: Long = -1) extends Arg(in_uniqueId){
  override def fullName(ctx: Component): String =
    if (mod eq ctx.id) name else s"${mod.getRef.name}.$name"
}
case class Slot(imm: Node, name: String, in_uniqueId: Long = -1) extends Arg(in_uniqueId){
  override def fullName(ctx: Component): String =
    if (imm.fullName(ctx).isEmpty) name else s"${imm.fullName(ctx)}.${name}"
}
case class Index(imm: Arg, value: Arg, in_uniqueId: Long = -1) extends Arg(in_uniqueId){
