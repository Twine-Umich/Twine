  ...
  def computeName(defaultPrefix: Option[String], defaultSeed: Option[String]): Option[String] = {
    ...
    def buildAggName(id: HasId): Option[String] = {
      def recArg(node: Arg): Option[String] = node match {
        case Slot(imm, name, _) => recArg(imm).map(_ + "_" + name)
        case Index(imm, ILit(num), _) => recArg(imm).map(_ + "_" + num)
        case Index(imm, n: LitArg, _) => recArg(imm).map(_ + "_" + n.num)
        case Index(imm, _: Node, _) => recArg(imm)
        case Node(id, _) => recArg(id.getOptionRef.get)
        case Ref(name, _) => Some(name)
        case ModuleIO(mod, name, _) if _parent.contains(mod) => Some(name)
        case ModuleIO(mod, name, _) => recArg(mod.getRef).map(_ + "_" + name)
      }
