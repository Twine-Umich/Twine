...
import chisel3.twine.internal._
...
//   }
// }
/** Abstract base class for TwineModule, which behave much like Verilog modules.
  * These may contain both logic and state which are written in the Module
  * body (constructor).
  * This abstract base class includes an implicit clock and reset.
  *
  * @note TwineModule instantiations must be wrapped in a Module() call.
  * @note TwineModule must implement one of the four TwineIOCtrl as ctrl.  
  */
abstract class TwineModule(implicit moduleCompileOptions: CompileOptions)
  extends TwineModuleInternal{
    override def ctrl: TwineIOCtrl
  ...
  * @group Connect
  */
  override def >>>[T <: TwineModuleInternal](that: T): T ={
    ...
    if(that.in.getElements == this.out.getElements){
      ...
      TwineTool.expandAndAddElements(that.in, input_ports)
      TwineTool.expandAndAddElements(this.out, output_ports)
      for((sink, i) <- input_ports.zipWithIndex){
        TwineTool.morphConnect(sink,output_ports(i))
      }
      if (!this.to_modules.contains(that)) this.to_modules += that
      if(!that.from_modules.contains(this)) that.from_modules += this
    }
      ...
      case d:DecoupledIOCtrl =>{
        ...
        if(d.size_of_receiving_buffer != 0){
          ...
          for((str,dat) <- this.in.elements){
            TwineTransformer.replaceUses(dat.ref, input_buffer.io.deq.bits.elements(str), this._commands)
            (dat, input_buffer.io.deq.bits.elements(str)) match{
              case (v_l: Vec[_], v_r: Vec[_]) =>{
                for(i <- 0 until v_l.length){
                  TwineTransformer.replaceUses(v_l(i).ref, v_r(i), this._commands)
                }
              }
          ...
          TwineTransformer.replaceUses(d.in.valid.ref, input_buffer.io.deq.valid, this._commands)
          TwineTransformer.replaceAll(d.in.ready.ref, input_buffer.io.deq.ready, this._commands)
        ...
        if(d.size_of_sending_buffer != 0){
          ...
          for((str,dat) <- this.out.elements){
            TwineTransformer.replaceAll(dat.ref, output_buffer.io.enq.bits.elements(str), this._commands)
            (dat, output_buffer.io.enq.bits.elements(str)) match{
              case (v_l: Vec[_], v_r: Vec[_]) =>{
                for(i <- 0 until v_l.length){
                  TwineTransformer.replaceAll(v_l(i).ref, v_r(i), this._commands)
                }
              }
              case _ => ()
            }
          }
          TwineTransformer.replaceAll(d.out.valid.ref, output_buffer.io.enq.valid, this._commands)
          TwineTransformer.replaceUses(d.out.ready.ref, output_buffer.io.enq.ready, this._commands)
        ...
        if(d.size_of_reorder_buffer != 0){
          ...
          if(input_uses_inorder){
            ...
            for((str,d) <- this.in.elements){
              TwineTransformer.replaceAll(d.ref, input_buffer.out.elements(str), this._commands)
            }
            TwineTransformer.replaceAll(d.in.valid.ref, input_buffer.ctrl.out.valid, this._commands)
            TwineTransformer.replaceAll(d.in.ready.ref, input_buffer.ctrl.out.ready, this._commands)
            TwineTransformer.replaceAll(d.in.ticket_num.ref, input_buffer.ctrl.out.ticket_num, this._commands)
          ...
          if(output_uses_inorder){
            ...
            for((str,d) <- this.out.elements){
              TwineTransformer.replaceAll(d.ref, output_buffer.in.elements(str), this._commands)
            }
            TwineTransformer.replaceAll(d.in.valid.ref, output_buffer.ctrl.in.valid, this._commands)
            TwineTransformer.replaceAll(d.in.ready.ref, output_buffer.ctrl.in.ready, this._commands)
            TwineTransformer.replaceAll(d.in.ticket_num.ref, output_buffer.ctrl.in.ticket_num, this._commands)
  ...
  private[chisel3] override def generateComponent(): Component = {
    ...
    require(ctrl != null, "Twine Module must have ctrl")
        ...
        (sink_sub, this.out.getElements(i)) match{
          ...
          case(sink_e: Element, source_vec: Vec[Data @unchecked]) =>{
            if(source_vec.length > 1){
              ...
              TwineTool.morphConnect(sink_sub, source_vec(0))
              if(sink_sub._parent.isDefined){
                sink_sub._parent.get match{
                  case sm:TwineModuleInternal =>{
                    if(!sm.sub_modules.contains(this)){
          ...
          case _ =>{
            TwineTool.morphConnect(sink_sub, this.out.getElements(i))
            if(sink_sub._parent.isDefined){
              sink_sub._parent.get match{
                case sm:TwineModuleInternal =>{
                  if(!sm.sub_modules.contains(this)){
...
/** Abstract base class for TwineState, which only contains basic sequential blocks.
  * These may contain both logic and state which are written in the Module
  * body (constructor).
  * This abstract base class includes an implicit clock and reset.
  *
  * @note TwineState instantiations must be wrapped in a State() call.
  * @note TwineState must implement one of the four TwineIOCtrl as ctrl.
  */
abstract class TwineState(implicit moduleCompileOptions: CompileOptions)
  extends TwineModule{}

/** Abstract base class for TwineLogic, which only contains basic combinational blocks.
  * These may contain logic which are written in the Module
  * body (constructor).
  * This abstract base class includes an implicit clock and reset.
  *
  * 
  * @note TwineLogic instantiations must be wrapped in a Logic() call.
  * @note TwineLogic must implement one of the four TwineIOCtrl as ctrl.
  */
abstract class TwineLogic(implicit moduleCompileOptions: CompileOptions)
  extends TwineModule{}